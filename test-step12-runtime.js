...
(async()=>{
  ...
  ok(syncPayload?.device?.appVersion==='10.130','device app version missing');
  ...
  ok(core.sanitizeDeviceMeta({label:'  iPhone   Perso  ',platform:'ios',standalone:1,appVersion:'10.130'}).label==='iPhone Perso','device metadata label sanitization failed');
  ...
  ok(html.includes('web-push-manager.js?v=10.130')&&sw.includes('web-push-manager.js?v=10.130'),'v10.130 Web Push manager not in PWA asset chain');
  ok(sw.includes("kinetik-v10-130-multidevice-account"),'v10.130 cache name missing');
  ok(pkg.version==='10.130.0','package version not updated');
