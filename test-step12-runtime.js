...
(async()=>{
  ...
  ok(manager?.version==='1.1.0','health manager version is incorrect');
  ...
  ok(syncPayload?.device?.appVersion==='10.126','device app version missing');
  ...
  ok(core.sanitizeDeviceMeta({label:'  iPhone   Perso  ',platform:'ios',standalone:1,appVersion:'10.126'}).label==='iPhone Perso','device metadata label sanitization failed');
  ...
  ok(deliver.includes('lastDeliveryAcceptedAt')&&deliver.includes('consecutiveFailures'),'delivery health persistence missing');
  ...
  ok(html.includes('web-push-manager.js?v=10.126')&&sw.includes('web-push-manager.js?v=10.126'),'v10.126 Web Push manager not in PWA asset chain');
  ok(sw.includes("kinetik-v10-126-push-health"),'v10.126 cache name missing');
  ok(pkg.version==='10.126.0','package version not updated');
