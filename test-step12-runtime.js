...
(async()=>{
  ...
  ok(manager?.version==='1.3.0','health manager version is incorrect');
  ...
  ok(syncPayload?.device?.appVersion==='10.131','device app version missing');
  ...
  ok(core.sanitizeDeviceMeta({label:'  iPhone   Perso  ',platform:'ios',standalone:1,appVersion:'10.131'}).label==='iPhone Perso','device metadata label sanitization failed');
  ...
  const sw=fs.readFileSync(__dirname+'/sw.js','utf8'),html=fs.readFileSync(__dirname+'/index.html','utf8'),appText=loadAppSource(__dirname),statusApi=fs.readFileSync(__dirname+'/api/push/status.js','utf8'),deliver=fs.readFileSync(__dirname+'/api/push/deliver.js','utf8'),testApi=fs.readFileSync(__dirname+'/api/push/test.js','utf8'),syncApi=fs.readFileSync(__dirname+'/api/push/sync.js','utf8'),pkg=JSON.parse(fs.readFileSync(__dirname+'/package.json','utf8'));
  ...
  ok(html.includes('web-push-manager.js?v=10.131')&&sw.includes('web-push-manager.js?v=10.131'),'v10.131 Web Push manager not in PWA asset chain');
  ok(sw.includes("kinetik-v10-131-integrity-lot-a"),'v10.131 cache name missing');
  ok(pkg.version==='10.131.0','package version not updated');
