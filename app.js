...
function cycleAiPromptText(c,goal){
  const cs=getCycleState(),profile=getProfile?getProfile():null,setup=getEquipmentSetup?getEquipmentSetup():null;
  const equipment=setup?Object.entries(setup).filter(([k,v])=>v===true||Array.isArray(v)&&v.length).map(([k,v])=>Array.isArray(v)?`${k}: ${v.join(', ')}`:k).join(', '):'Utilise uniquement le matériel indiqué dans le programme fourni.';
  ...
  return `Tu es un coach spécialisé en calisthénie et en programmation de l’entraînement. Aide-moi à adapter mon cycle sans changer inutilement les exercices. Je veux une progression réaliste, mesurable et prudente.\n\nOBJECTIF UTILISATEUR\n${goal||'[À préciser]'}\n\nDONNÉES DE CALISTHENIE COACH\nCycle: ${c.name}\nPhase actuelle: ${cs.name}\nSemaine actuelle: ${cs.week}/${cs.weekCount}\nMode actuel: ${progressionModeLabel(cs.plan)}\nVolume actuel: ${Math.round(cs.setFactor*100)}%\nCible reps/holds: ${Math.round(cs.targetFactor*100)}%\nRIR actuel: ${cs.rir}\nCardio: ${Math.round(cs.cardioFactor*100)}%\nMatériel détecté: ${equipment||'non renseigné'}\n\nCYCLE ACTUEL\n${weeks}\n\nTA MISSION\n1. Analyse si la structure du cycle correspond à mon objectif.\n2. Explique brièvement ce que tu changerais et pourquoi.\n3. Propose un cycle de ${cs.weekCount} semaines. Pour chaque semaine, donne: nom de phase, volume relatif en %, cible reps/holds en %, RIR, cardio en %, progression automatique oui/non.\n4. Prévois une gestion de fatigue/deload si pertinente.\n5. Ne recommande pas de progression agressive uniquement pour aller plus vite. Si des informations importantes manquent, pose-moi d’abord jusqu’à 5 questions ciblées avant de finaliser le cycle.\n\nÀ la fin, fournis aussi un tableau compact S1 à S${cs.weekCount} afin que je puisse reporter facilement les valeurs dans Calisthenie Coach.`;
}
...
function bindEvents(){
  ...
  const genAi=document.getElementById('generateCycleAiPrompt');if(genAi)genAi.onclick=()=>{const c=trainingCycleById(state.cycleProgressionEditor),goal=(document.getElementById('cycleAiGoal')?.value||'').trim(),out=document.getElementById('cycleAiOutput'),ta=document.getElementById('cycleAiPrompt');ta.value=cycleAiPromptText(c,goal);out.hidden=false;ta.focus();};
