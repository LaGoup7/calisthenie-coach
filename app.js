...
// V9.3.1 · Cycles + calendrier Semaine + Quick Log unifié + page Mesures dédiée.
...
async function exportBackup(){
  ...
  const backup={app:'Calisthenie Coach',schema:1,version:'9.3.1',exportedAt:new Date().toISOString(),data,photos};
...
function renderMore(){
  ...
  return shell(`<header class="topbar"><div><div class="brand">Plus</div><div class="daylabel">Outils & réglages · V9.3.1</div></div></header>
...
function bodyDelta(logs,key){
  const valid=(logs||[]).filter(x=>Number(x[key])>0);if(valid.length<2)return null;
  const delta=Number(valid[0][key])-Number(valid[1][key]);return Math.abs(delta)<.05?0:delta;
}
function bodyTrendText(value,unit,lowerIsBetter=false){
  if(value==null)return 'Pas assez de données';
  if(value===0)return 'Stable depuis le dernier relevé';
  const sign=value>0?'+':'−',abs=Math.abs(value).toFixed(1).replace('.0','');
  const good=lowerIsBetter?value<0:null;
  return `${sign}${abs} ${unit} depuis le dernier relevé${good===true?' · tendance ↓':good===false?' · tendance ↑':''}`;
}
function renderMeasurements(){
  const logs=getBodyLogs(),latest=logs[0],prev=logs[1],latestBf=latest?estimateBodyFat(latest.height,latest.waist,latest.neck):null;
  const weightDelta=bodyDelta(logs,'weight'),waistDelta=bodyDelta(logs,'waist'),neckDelta=bodyDelta(logs,'neck');
  const lastDate=latest?formatDate(latest.date):'Aucune mesure';
  const history=logs.slice(0,18);
  return shell(`<header class="topbar"><div><div class="brand">Mesures</div><div class="daylabel">Journal physique · ${logs.length} relevé${logs.length>1?'s':''}</div></div><button class="btn btn-primary compact" id="addBody">＋ Ajouter</button></header>
    <section class="card measurement-hero">
      <div><div class="kicker">Dernier relevé</div><h1>${lastDate}</h1><p class="muted">Suis les tendances plutôt qu'une valeur isolée. Les graphiques utilisent tes relevés locaux existants.</p></div>
      ${latest?`<div class="measurement-hero-value"><strong>${latest.weight?latest.weight+' kg':'—'}</strong><span>${latest.waist?latest.waist+' cm de taille':'Tour de taille non renseigné'}</span></div>`:''}
    </section>
    ${logs.length?`<section class="measurement-kpis">
      <article class="card measurement-kpi"><span>Poids</span><strong>${latest.weight?latest.weight+' kg':'—'}</strong><small>${bodyTrendText(weightDelta,'kg')}</small></article>
      <article class="card measurement-kpi"><span>Tour de taille</span><strong>${latest.waist?latest.waist+' cm':'—'}</strong><small>${bodyTrendText(waistDelta,'cm',true)}</small></article>
      <article class="card measurement-kpi"><span>Masse grasse estimée</span><strong>${latestBf?latestBf.toFixed(1)+' %':'—'}</strong><small>Estimation indicative à partir de taille / cou / taille</small></article>
      <article class="card measurement-kpi"><span>Tour de cou</span><strong>${latest.neck?latest.neck+' cm':'—'}</strong><small>${bodyTrendText(neckDelta,'cm')}</small></article>
    </section>
    <section class="card measurement-charts"><div class="section-head"><div><div class="kicker">Tendances</div><h2>Évolution récente</h2></div><span class="pill">12 relevés max</span></div>${renderBodyChart(logs,'weight','kg')}${renderBodyChart(logs,'waist','cm')}</section>
    ${latest.photoId?`<section class="card photo-card measurement-photo"><div class="section-head"><div><div class="kicker">Progression visuelle</div><h2>Dernière photo</h2></div></div><img id="latestBodyPhoto" alt="Photo de progression"><small>Photo enregistrée uniquement sur cet appareil.</small></section>`:''}
    <section class="card measurement-history"><div class="section-head"><div><div class="kicker">Historique</div><h2>Mes relevés</h2></div><span class="pill">${logs.length}</span></div><div class="body-history">${history.map((l,i)=>{const bf=estimateBodyFat(l.height,l.waist,l.neck);return `<div class="measurement-history-row"><div><strong>${formatDate(l.date)}</strong><small>${l.note?esc(l.note):i===0?'Dernier relevé':''}</small></div><span>${l.weight?l.weight+' kg':''}${l.weight&&l.waist?' · ':''}${l.waist?l.waist+' cm':''}${bf?' · '+bf.toFixed(1)+' %':''}${l.photoId?' · 📷':''}</span></div>`}).join('')}</div></section>`:
    `<section class="card measurement-empty"><div class="measurement-empty-icon">◉</div><h2>Commence ton suivi</h2><p class="muted">Ajoute ton poids, ton tour de taille et ton tour de cou. L'application créera automatiquement les tendances.</p><button class="btn btn-primary" id="addBodyEmpty">＋ Ajouter ma première mesure</button></section>`}
  `,'more');
}
...
function bindEvents(){
  ...
  const addBody=document.getElementById('addBody');if(addBody)addBody.onclick=()=>{state.bodyEditor=true;render();};const addBodyEmpty=document.getElementById('addBodyEmpty');if(addBodyEmpty)addBodyEmpty.onclick=()=>{state.bodyEditor=true;render();};
  ...
  if(state.view==='measurements'&&!state.bodyEditor)hydrateBodyPhoto();
}
