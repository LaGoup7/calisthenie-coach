...
function cycleAiPromptText(c,goal,opts={}){
  ...
  const recText=records.slice(0,12).map(r=>`- ${r.exercise}: ${recordValueText(r)} (${formatShortDate(r.date)})`).join('\n')||'- Aucun record enregistré';
  const recent=history.slice(-6).reverse().map(s=>{const entries=(s.entries||[]).filter(x=>x.type!=='timer').slice(0,8).map(x=>`${x.exercise}: ${recordValueText(x)}`).join('; ');return `- ${formatShortDate(s.date)}: ${entries||'séance enregistrée'}`}).join('\n')||'- Pas encore de séances exploitables';
...
function bindEvents(){
  ...
  const genAi=document.getElementById('generateCycleAiPrompt');if(genAi)genAi.onclick=()=>{const c=trainingCycleById(state.cycleProgressionEditor),goal=(document.getElementById('cycleAiGoal')?.value||'').trim(),opts={objective:document.getElementById('cycleAiObjective')?.value,target:(document.getElementById('cycleAiTarget')?.value||'').trim(),horizon:document.getElementById('cycleAiHorizon')?.value,context:document.getElementById('cycleAiContext')?.value},out=document.getElementById('cycleAiOutput'),ta=document.getElementById('cycleAiPrompt');ta.value=cycleAiPromptText(c,goal,opts);out.hidden=false;ta.focus();};
