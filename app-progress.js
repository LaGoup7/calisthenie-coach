...
/* V10.153 · Athlete Passport — immersive profile direction */
renderMore=function(){
  const p=getAthleteProfile(),cycle=getActiveTrainingCycle(),cs=getCycleState(),status=v10149ProfileStatus(p),rank=getRankState();
  const goal=v10150GoalPath(p.primaryGoal),body=getBodyLogs(),latest=body[0],weight=latest?bodyValue(latest,'weight'):p.weight,waist=latest?bodyValue(latest,'waist'):null;
  const mobility=mobilityProfiles(),mobilityDone=mobility.filter(x=>x.assessed).length,setup=getEquipmentSetup(),equipment=EQUIPMENT_CATALOG.filter(x=>setup[x.id]),days=p.trainingDays||[];
  const restrictions=typeof getRestrictions==='function'?Object.entries(getRestrictions()).filter(([,v])=>v).map(([id])=>restrictionLabel(id)):[];
  ...
  return shell(`<div class="p153-world rank-${rank.current.id}">
    ...
    <section class="p153-telemetry"><button data-view="measurements"><span>MASSE</span><strong>${weight?Number(weight).toFixed(1):'—'}</strong><small>KG</small></button><button data-view="measurements"><span>TAILLE</span><strong>${waist?Number(waist).toFixed(0):'—'}</strong><small>CM / TOUR</small></button><button data-view="skills"><span>RANG</span><strong>${rank.current.name.slice(0,3).toUpperCase()}</strong><small>${rank.nextEval?`${rank.nextEval.completed}/${rank.nextEval.required} VALIDÉS`:'COMPLET'}</small></button><button data-view="flexibility"><span>MOBILITÉ</span><strong>${mobilityDone}/${mobility.length}</strong><small>ZONES</small></button><button data-view="week"><span>RYTHME</span><strong>${p.weeklySessions}×</strong><small>${p.preferredDuration} MIN</small></button></section>
    <section class="p153-operation-grid"><article class="p153-cycle"><div class="p153-section-label"><span>01</span><strong>CYCLE</strong></div><h3>${esc(cycle.name)}</h3><div class="p153-cycle-week"><strong>S${cs.week}</strong><span>/ ${cs.weekCount}</span></div><div class="p153-cycle-track"><i style="width:${Math.min(100,cs.week/cs.weekCount*100)}%"></i></div><button data-view="week">OUVRIR LE PROGRAMME ↗</button></article><article class="p153-week"><div class="p153-section-label"><span>02</span><strong>DISPONIBILITÉ</strong></div><div class="p153-week-track">${[1,2,3,4,5,6,0].map(d=>`<div class="${days.includes(d)?'on':''}"><b>${athleteDayLabel(d).slice(0,1)}</b><i></i></div>`).join('')}</div><div class="p153-week-legend"><span><i></i> ENTRAÎNEMENT</span><strong>${days.length} JOURS</strong></div><button data-edit-athlete-profile>MODIFIER ↗</button></article></section>
    <section class="p153-gear"><div class="p153-section-label"><span>03</span><strong>ARSENAL</strong><em>${equipment.length} PIÈCES ACTIVES</em></div><div>${equipment.slice(0,10).map((x,i)=>`<button data-edit-athlete-profile><b>${String(i+1).padStart(2,'0')}</b>${equipmentVisualIcon(x.label)}<span>${esc(x.label)}</span></button>`).join('')||'<button data-edit-athlete-profile class="empty"><strong>＋</strong><span>AJOUTER DU MATÉRIEL</span></button>'}</div></section>
    ${restrictions.length?`<section class="p153-caution"><strong>!</strong><span>ZONES SOUS SURVEILLANCE</span><div>${restrictions.map(x=>`<b>${esc(x)}</b>`).join('')}</div><button data-edit-athlete-profile>ÉDITER</button></section>`:''}
    <section class="p153-vault"><div class="p153-section-label"><span>04</span><strong>SYSTÈME</strong></div><button data-view="measurements"><span>${uiIcon('measurements')}</span><div><small>CORPS</small><strong>${body.length} relevé${body.length>1?'s':''}</strong></div><b>${latest?formatDate(latest.date):'DÉMARRER'} →</b></button><button data-view="settings"><span>${uiIcon('clock')}</span><div><small>PROTOCOLES</small><strong>Timers & écran</strong></div><b>OUVRIR →</b></button><button data-view="settings"><span>${account?.linked?'✓':'⌁'}</span><div><small>DONNÉES</small><strong>${account?.linked?'Synchronisation active':'Stockage local'}</strong></div><b>GÉRER →</b></button>${state.stravaStatus.checked&&!state.stravaStatus.connected?`<a href="/api/strava/auth"><span>S</span><div><small>CONNEXION</small><strong>Strava</strong></div><b>CONNECTER ＋</b></a>`:`<button data-view="settings"><span>S</span><div><small>CONNEXION</small><strong>Strava</strong></div><b>${state.stravaStatus.connected?'ACTIF':'…'}</b></button>`}</section>
  </div>`, 'athlete');
};
