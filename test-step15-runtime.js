...
(async()=>{
  ...
  const members=()=>[{deviceId:deviceId||'pending',deviceIdSuffix:String(deviceId||'pending').slice(-8),current:true,label:'Mon PC',platform:'windows',standalone:false,appVersion:'10.131',joinedAt:'2026-09-01T10:00:00Z',lastSeenAt:'2026-09-01T10:01:00Z',notificationsMuted:false,pushLinked:false,pushInstallationSuffix:null}];
  ...
  let r=await run(statusHandler,{action:'create',deviceId:d1,deviceSecret:ds1,device:{label:'iPhone',platform:'ios',standalone:true,appVersion:'10.131'},pushInstallationId:p1,pushDeviceSecret:ps1});ok(r.status===200&&r.data.accountId,'server account create failed');const aid=r.data.accountId;
  ...
  ok(html.includes('account-manager.js?v=10.131')&&sw.includes('account-manager.js?v=10.131'),'account manager missing from PWA chain');ok(html.indexOf('account-manager.js?v=10.131')>html.indexOf('web-push-manager.js?v=10.131'),'account manager load order is incorrect');ok(sw.includes('kinetik-v10-131-integrity-lot-a'),'v10.131 cache missing');ok(pkg.version==='10.131.0','package version mismatch');
