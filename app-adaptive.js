/* KINETIK v10.141 · Adaptive coaching, assessment and progression intelligence. */
...
renderActivityEditor=function(){const editing=state.activityEditId?activityById(state.activityEditId):null,preset=!editing?state.activityPresetData:null,type=activityType(editing?.type||preset?.type||state.activityPresetType||'running'),date=(editing?.date||new Date().toISOString()).slice(0,10),duration=Number(editing?.duration??preset?.duration??30),distance=editing?.distance??preset?.distance??'',rpe=Number(editing?.rpe??preset?.rpe??5),note=editing?.note||'';return shell(`<header class="topbar activity-topbar"><div><div class="brand">${editing?'Modifier l’activité':'Nouvelle activité'}</div><div class="daylabel">Toutes tes disciplines au même endroit</div></div></header><section class="activity-editor activity-editor-premium"><div class="activity-editor-intro"><div class="activity-editor-symbol" id="activityEditorSymbol">${activityUiIcon(type.id)}</div><div><div class="kicker">${editing?'Édition':preset?'Reprendre une activité':'Activité'}</div><h1 id="activityEditorTitle">${esc(type.label)}</h1><p>${preset?'Dernières valeurs reprises. Modifie seulement ce qui a changé.':'KINETIK combine la durée et l’effort perçu pour suivre la charge. La mobilité reste comptée comme récupération.'}</p></div></div><div class="activity-form-section activity-editor-two"><label class="activity-field"><span>Type</span><select id="activityType">${ACTIVITY_TYPES.map(x=>`<option value="${x.id}" ${x.id===type.id?'selected':''}>${x.label}</option>`).join('')}</select></label><label class="activity-field"><span>Date</span><input id="activityDate" type="date" value="${date}"></label></div><div class="activity-metrics-grid"><label class="activity-metric"><span>Durée</span><div><input id="activityDuration" type="number" min="1" value="${duration}"><b>min</b></div></label><label class="activity-metric" id="activityDistanceWrap"><span>Distance</span><div><input id="activityDistance" type="number" min="0" step=".1" value="${distance}"><b id="activityDistanceUnit">${type.metric||'km'}</b></div></label></div><div class="activity-form-section activity-rpe-section"><div class="activity-rpe-head"><div><span>Effort perçu</span><small>1 très facile · 10 maximal</small></div><strong id="activityRpeValue">${rpe}</strong></div><input id="activityRpe" class="activity-rpe-slider" type="range" min="1" max="10" value="${rpe}"></div><div class="activity-form-section"><label class="activity-field"><span>Note</span><textarea id="activityNote" rows="3">${esc(note)}</textarea></label></div><div class="activity-load-preview activity-load-premium"><div><span>${type.id==='mobility'?'Récupération':'Charge de la séance'}</span><small>${type.id==='mobility'?'suivie séparément':'durée × effort perçu'}</small></div><strong id="activityLoadPreview">${type.id==='mobility'?'—':Math.round(duration*rpe)+' <small>indice</small>'}</strong></div><div class="activity-editor-actions"><button class="btn activity-cancel" id="cancelActivity">Annuler</button><button class="btn activity-save" id="saveActivity">${editing?'Enregistrer':'Ajouter'}</button></div></section>`,'today');};

function v10140TrainingTimeLabel(minutes){
  const mins=Math.max(0,Math.round(Number(minutes)||0));
  if(mins<60)return `${mins} min`;
  const h=Math.floor(mins/60),m=mins%60;
  return `${h} h${m?` ${String(m).padStart(2,'0')}`:''}`;
}
renderActivityHub=function(){
  const all=totalTrainingStats(7),s=all.activities;
  const rows=dailyVolumeRows(),quickToday=getQuickLogs().filter(x=>localDateKey(x.date)===localDateKey());
  const quickReps=quickToday.reduce((n,x)=>n+repsEquivalent(x.type,x.value),0);
  const relevant=rows.filter(r=>r.quick>0||r.guided>0),warning=dailyQuickLoadWarning();
  const categories=[['Calisthénie / force',all.strength.minutes],...Object.entries(s.rows).filter(([name])=>name!=='Mobilité / étirements').map(([name,v])=>[name,v.minutes])].filter(([,minutes])=>minutes>0);
  const max=Math.max(1,...categories.map(([,minutes])=>minutes));
  const emptyToday=quickToday.length===0&&!relevant.length;
  return `<section class="card today-activity-summary"><div class="section-head today-activity-head"><div><div class="kicker">Activité du jour</div><h2>Aujourd’hui</h2><p class="muted small">Vue rapide aujourd’hui · aperçu sur 7 jours</p></div><button class="btn btn-secondary compact" type="button" data-open-add-hub="true">＋ Ajouter</button></div>
    <div class="today-activity-kpis">
      <div><strong>${quickReps}</strong><span>Répétitions ajoutées</span><small>aujourd’hui</small></div>
      <div><strong>${quickToday.length}</strong><span>Séries rapides</span><small>aujourd’hui</small></div>
      <div><strong>${all.sessions}</strong><span>Séance${all.sessions!==1?'s':''}</span><small>sur 7 jours</small></div>
      <div><strong>${Math.round(all.recoveryMinutes)}</strong><span>Récupération</span><small>min · sur 7 jours</small></div>
    </div>
    <div class="today-activity-week"><div><span>Entraînement récent</span><strong>${v10140TrainingTimeLabel(all.minutes)}</strong></div><small>Temps total enregistré sur les 7 derniers jours.</small></div>
    ${emptyToday?'<p class="muted small today-activity-empty">Aucune activité enregistrée aujourd’hui. Utilise + Ajouter pour noter une séance, une activité ou quelques séries.</p>':''}
    ${(relevant.length||categories.length)?`<details class="today-activity-details"><summary><span>Voir le détail</span><b>⌄</b></summary><div class="today-activity-detail-body">${relevant.length?`<div class="today-activity-detail-section"><strong>Aujourd’hui</strong>${relevant.map(r=>{const unit=quickUnit(r.type),total=r.quick+r.guided;return `<div class="today-activity-detail-row"><span>${esc(r.family)}</span><small>${r.quick?`${r.quick} ${unit} ajoutés`:''}${r.quick&&r.guided?' · ':''}${r.guided?`${r.guided} ${unit} en séance`:''}</small><b>${total} ${unit}</b></div>`;}).join('')}</div>`:''}${categories.length?`<div class="today-activity-detail-section"><strong>Répartition · 7 jours</strong>${categories.map(([name,min])=>`<div class="today-activity-mix-row"><div><span>${esc(name)}</span><b>${Math.round(min)} min</b></div><i><em style="width:${Math.max(4,Math.round(min/max*100))}%"></em></i></div>`).join('')}</div>`:''}</div></details>`:''}
    ${warning?`<div class="volume-warning">⚠ ${warning}</div>`:''}
  </section>`;
};
renderDailyVolumeCard=function(){return '';};
